...
for _ in range(n):  # 노드 개수동안 반복
    # 입력 예시 : A B C -> B None None -> C None None
    data, left_node, right_node = input().split()
    if left_node == "None":  # 왼쪽 노드가 'None'이면
        left_node = None  # type:ignore
    if right_node == "None":  # 오른쪽 노드가 'None'이면
        right_node = None  # type:ignore
    tree[data] = Node(
        data, left_node, right_node
    )  # {'A' : (A, B, C), 'B' : (B, None, None), 'C' : (C, None, None)}


# 전위 순회
def pre_order(node: Node):
    print(node.data, end=" ")  # node의 data를 출력한다.
    if node.left_node is not None:  # 노드의 왼쪽 자식이 None이 아니면
...
def in_order(node: Node):
    if node.left_node is not None:  # 루트 노드(A)는 왼쪽 자식(B)이 None이 아니므로
    ...
    if node.right_node is not None:  # 루트노드의 오른쪽 자식은 None이 아니므로
